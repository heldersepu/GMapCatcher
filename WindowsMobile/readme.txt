...
Only solution is to edit file '<storage card>\wmgooglemaps\lastmap.txt' and set existing map or rename one of existing maps so the name in '<storage card>\wmgooglemaps\lastmap.txt' is valid.
...
- while doubleclicking the map zoom is in fact changed and map is displayed with bigger zoom but zoom level displayed in buttons [+] [-] doesn't show the new value.
